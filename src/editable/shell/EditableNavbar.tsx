...
export function EditableNavbar() {
  ...
  const navVars = { '--editable-nav-bg': '#ffffff', '--editable-nav-text': '#07152e', '--editable-nav-active': '#2563c8', '--editable-nav-active-text': '#ffffff', '--editable-cta-bg': '#2563c8', '--editable-cta-text': '#ffffff', '--editable-search-bg': '#f3f7fb', '--editable-border': `${preset.colors.muted}33`, '--editable-container': '1180px' } as CSSProperties
      ...
      <nav className="mx-auto flex min-h-[88px] w-full max-w-[var(--editable-container)] items-center gap-4 px-4 sm:px-6 lg:px-8">
        <Link href="/" className="group flex shrink-0 items-center gap-3">
          ...
          <span className="hidden min-w-0 sm:block">
            <span className="block max-w-[180px] truncate text-sm font-black tracking-[-0.03em]">{globalContent.site.name}</span>
            <span className="block max-w-[180px] truncate text-[11px] font-bold uppercase tracking-[0.18em] opacity-55">{globalContent.nav?.tagline || globalContent.site.tagline}</span>
          </span>
        </Link>
        ...
        <div className="ml-auto flex shrink-0 items-center gap-2">
          {session ? (
            <>
              ...
              <span className="hidden max-w-[140px] truncate rounded-full border border-[var(--editable-border)] bg-[var(--editable-search-bg)] px-4 py-2 text-sm font-black sm:inline-flex">{session.name}</span>
